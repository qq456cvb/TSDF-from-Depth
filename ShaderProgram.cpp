...
static bool CheckShaderStatus(GLuint shader)
{
    ...
    if (status == 0)
    {
        ...
        if (messageLength > 1)
        {
            ...
            std::cerr << "GLSL compiler message: " << message.get() << "\n";
        }
        return false;
    }
                                    ...
                                    const char *fragmentShaderFileName)
{
    ...
    if (!vertexShaderFile)
    {
        std::cerr << "unable to open \"" << vertexShaderFileName << "\"!\n";
        return false;
    }
    ...
    {
        std::cerr << "unable to open \"" << fragmentShaderFileName << "\"!\n";
        return false;
    }
		...
		const char *attribNames[], int *attribIds[], int attribCount)
{
    ...
    if (!mLoaded)
    {
        std::cerr << "Error loading " << vertexShaderFilename << " or " << fragmentShaderFilename << "\n";
        return false;
    }
